<?php
/**
 * LottoExpert.net — Arizona The Pick Results Intelligence Page
 * Joomla 5.x + PHP 8.1+
 *
 * PURPOSE
 ...
 * - Production-safe query handling
 *
 ...
 * - sixth ball range = 01..41 (same pool)
 */
...
if (isset($stateName, $gName) && $stateName !== '' && $gName !== '') {
    ...
    $doc->setMetaData('description', 'Explore ' . $stateName . ' ' . $gName . ' frequency data, hot and cold numbers, draw history, and recency analysis. Data-driven lottery insights — no hype.');
}
 ...
 * --------------------------------------------------------------------------
 */
function aztFmtDate(?string $date): string
{
    if (!$date) {
        return '';
    }
...
function aztFmtDateLong(?string $date): string
{
    if (!$date) {
        return '—';
    }
...
function aztPad2(string $value): string
{
...
function aztResolveLogo(string $stateAbrev, string $gName): array
{
...
function aztInitRange(int $min, int $max): array
{
    $counts = [];
    $lastSeenIndex = [];
...
function aztDrawingsAgoLabel(?int $idx, int $window): array
{
    if ($idx === null) {
...
function aztBuildNaturalLabels(int $min, int $max): array
{
    $labels = [];
...
function aztTopKeysByValue(array $counts, int $limit, bool $ascending = false): array
{
    $work = $counts;
...
function aztFindRepeatedFromLatest(array $latestBalls, array $previousRows): array
{
    $repeated = [];
...
function aztCommaList(array $items): string
{
...
function aztFetchRecentDraws(\Joomla\Database\DatabaseDriver $db, string $dbCol, string $gameId, int $limit): array
{
...
function aztGetPreviousOccurrenceDate(
...
function aztGetDrawingsSinceDate(
    \Joomla\Database\DatabaseDriver $db,
    string $dbCol,
    string $gameId,
    ?string $previousDate,
...
function aztEscapeJsString(string $value): string
{
    return str_replace(
        ["\\", "'", "\r", "\n", '</'],
        ["\\\\", "\\'", '', '', '<\/'],
...
$defaultWindowSixth = 100;

$nodCurrentMain  = $defaultWindowMain;
$nodCurrentSixth = $defaultWindowSixth;
...
if ($input->getMethod() === 'POST' && Session::checkToken()) {
    ...
    if ($input->post->get('pfq-search', null, 'cmd') !== null) {
        $nodCurrentSixth = (int) $input->post->get('pnod', $defaultWindowSixth, 'int');
    }
}

$nodCurrentMain  = max(10, min(700, $nodCurrentMain));
$nodCurrentSixth = max(10, min(700, $nodCurrentSixth));
 ...
 * --------------------------------------------------------------------------
 */
$latestRows = aztFetchRecentDraws($db, (string) $dbCol, $gameId, 1);
...
$logo = (isset($stateAbrev, $gName)) ? aztResolveLogo((string) $stateAbrev, (string) $gName) : ['exists' => false, 'url' => ''];

$rowsMain  = aztFetchRecentDraws($db, (string) $dbCol, $gameId, $nodCurrentMain);
$rowsSixth = ($nodCurrentSixth === $nodCurrentMain) ? $rowsMain : aztFetchRecentDraws($db, (string) $dbCol, $gameId, $nodCurrentSixth);

[$mainCounts, $mainLastSeenIndex]   = aztInitRange(1, 41);
[$sixthCounts, $sixthLastSeenIndex] = aztInitRange(1, 41);
...
foreach ($rowsSixth as $idx => $row) {
    $ball = trim((string) ($row['sixth'] ?? ''));

    if ($ball === '' || !isset($sixthCounts[$ball])) {
        continue;
    }

    $sixthCounts[$ball]++;

    if ($sixthLastSeenIndex[$ball] === null) {
        $sixthLastSeenIndex[$ball] = (int) $idx;
    }
}
 ...
 * --------------------------------------------------------------------------
 */
$mainChartLabels  = aztBuildNaturalLabels(1, 41);
$mainChartValues  = [];
$mainRecencyValues = [];
...
$sixthChartLabels = aztBuildNaturalLabels(1, 41);
$sixthChartValues = [];

foreach ($sixthChartLabels as $label) {
    $sixthChartValues[] = (int) ($sixthCounts[$label] ?? 0);
}

$topActiveKeys  = aztTopKeysByValue($mainCounts, 10, false);
$topQuietKeys   = aztTopKeysByValue($mainCounts, 10, true);
...
$repeatedNumbers    = aztFindRepeatedFromLatest($latestMainBalls, array_slice($rowsMain, 1, 10));
$mostActiveSummary  = array_slice($topActiveKeys, 0, 3);
$quietSummary       = array_slice($quietestKeys, 0, 3);

$window50  = aztFetchRecentDraws($db, (string) $dbCol, $gameId, 50);
$window300 = aztFetchRecentDraws($db, (string) $dbCol, $gameId, 300);

[$counts50, ]  = aztInitRange(1, 41);
[$counts300, ] = aztInitRange(1, 41);
...
$top50  = aztTopKeysByValue($counts50, 5, false);
$top300 = aztTopKeysByValue($counts300, 5, false);
...
$windowChangeNarrative  = 'In the recent 50-draw view, the leading activity centers on ' . aztCommaList(array_slice($top50, 0, 3)) . '. ';
$windowChangeNarrative .= 'In the broader 300-draw view, ' . aztCommaList(array_slice($top300, 0, 3)) . ' remains more historically prominent. ';
if (!empty($windowShiftIn)) {
    $windowChangeNarrative .= aztCommaList(array_slice($windowShiftIn, 0, 2)) . ' gains prominence when the window broadens. ';
}
if (!empty($windowShiftOut)) {
    $windowChangeNarrative .= aztCommaList(array_slice($windowShiftOut, 0, 2)) . ' looks more concentrated in the shorter recent view.';
}
...
if ($drawDate !== '') {
    foreach ($latestMainBalls as $ball) {
        $prevDate = aztGetPreviousOccurrenceDate($db, (string) $dbCol, $gameId, $drawDate, $ball, false);
        $drawsAgo = aztGetDrawingsSinceDate($db, (string) $dbCol, $gameId, $prevDate, $drawDate);
    ...
    $prevSixthDate = aztGetPreviousOccurrenceDate($db, (string) $dbCol, $gameId, $drawDate, $pb, true);
    $sixthDrawsAgo = aztGetDrawingsSinceDate($db, (string) $dbCol, $gameId, $prevSixthDate, $drawDate);

    $drawHistoryRows[] = [
        'label'    => 'Sixth Ball (' . aztPad2($pb) . ')',
        'prevDate' => $prevSixthDate,
        'drawsAgo' => $sixthDrawsAgo,
        'isBonus'  => true,
    ];
}
 ...
 * --------------------------------------------------------------------------
 */
$schemaStateName = htmlspecialchars((string) ($stateName ?? ''), ENT_QUOTES, 'UTF-8');
$schemaGameName  = htmlspecialchars((string) ($gName ?? ''), ENT_QUOTES, 'UTF-8');
$schemaUrl       = htmlspecialchars($canonicalNoQuery, ENT_QUOTES, 'UTF-8');

$schemaJson = json_encode([
    '@context'    => 'https://schema.org',
    '@type'       => 'WebPage',
    'name'        => $schemaStateName . ' ' . $schemaGameName . ' Results Intelligence',
    'description' => 'Frequency analysis, hot and cold numbers, draw history, and recency data for ' . $schemaStateName . ' ' . $schemaGameName . '.',
...
$doc->addCustomTag('<script type="application/ld+json">' . ($schemaJson !== false ? $schemaJson : '{}') . '</script>');
?>
<style>
:root{
  --skai-blue:#1C66FF;
      ...
      <div class="skai-hero-top">
        ...
        <div class="skai-hero-copy">
          ...
          <div class="skai-ball-row" aria-label="Latest drawn numbers">
            <span class="skai-ball skai-ball--main"><?php echo htmlspecialchars(aztPad2($p1), ENT_QUOTES, 'UTF-8'); ?></span>
            <span class="skai-ball skai-ball--main"><?php echo htmlspecialchars(aztPad2($p2), ENT_QUOTES, 'UTF-8'); ?></span>
            <span class="skai-ball skai-ball--main"><?php echo htmlspecialchars(aztPad2($p3), ENT_QUOTES, 'UTF-8'); ?></span>
            <span class="skai-ball skai-ball--main"><?php echo htmlspecialchars(aztPad2($p4), ENT_QUOTES, 'UTF-8'); ?></span>
            <span class="skai-ball skai-ball--main"><?php echo htmlspecialchars(aztPad2($p5), ENT_QUOTES, 'UTF-8'); ?></span>
            <span class="skai-ball-gap" aria-hidden="true"></span>
            <span class="skai-ball skai-ball--bonus"><?php echo htmlspecialchars(aztPad2($pb), ENT_QUOTES, 'UTF-8'); ?></span>
          </div>

          <div class="skai-hero-actions" aria-label="Primary actions">
            <a class="skai-btn skai-btn--primary" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=AZT">
              Open SKAI Analysis
            </a>
            <a class="skai-btn skai-btn--secondary" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=AZT">
              AI Predictions
            </a>
          ...
          <div class="skai-advanced-links" aria-label="Advanced tools">
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=AZT">Skip &amp; Hit Analysis</a>
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=AZT">MCMC Markov Analysis</a>
            <a class="skai-mini-link" href="/all-lottery-heatmaps?gameId=AZT">Heatmap Analysis</a>
          </div>
        </div>
          ...
          <div class="skai-meta-stack">
            <div class="skai-meta-row">
              <div class="skai-meta-box">
                ...
                <span class="value"><?php echo htmlspecialchars(aztFmtDateLong($drawDate), ENT_QUOTES, 'UTF-8'); ?></span>
              </div>
              <div class="skai-meta-box">
                <span class="label">Next draw date</span>
                <span class="value"><?php echo htmlspecialchars(aztFmtDateLong($nextDrawDate), ENT_QUOTES, 'UTF-8'); ?></span>
              </div>
            </div>
  ...
  <section class="skai-strip" aria-label="Key takeaways">
    <article class="skai-stat">
      ...
      <div class="skai-stat-body">
        <div class="skai-stat-value"><?php echo htmlspecialchars(aztCommaList($mostActiveSummary), ENT_QUOTES, 'UTF-8'); ?></div>
    ...
    <article class="skai-stat">
      ...
      <div class="skai-stat-body">
        <div class="skai-stat-value"><?php echo htmlspecialchars(aztCommaList($quietSummary), ENT_QUOTES, 'UTF-8'); ?></div>
    ...
    <article class="skai-stat">
      ...
      <div class="skai-stat-body">
        <div class="skai-stat-value"><?php echo htmlspecialchars(!empty($repeatedNumbers) ? aztCommaList($repeatedNumbers) : 'None', ENT_QUOTES, 'UTF-8'); ?></div>
    ...
    <article class="skai-stat">
      ...
      <div class="skai-stat-body">
        <div class="skai-stat-value"><?php echo (int) $nodCurrentMain; ?> / <?php echo (int) $nodCurrentSixth; ?></div>
        <div class="skai-stat-note">Main-number and Sixth Ball draw windows currently loaded for this page view.</div>
      </div>
    </article>
  </section>
                ...
                <div class="skai-history-item">
                  ...
                  <div class="skai-history-date">
                    <?php if (!empty($row['prevDate'])) : ?>
                      Previously seen on <?php echo htmlspecialchars(aztFmtDateLong((string) $row['prevDate']), ENT_QUOTES, 'UTF-8'); ?>
                    <?php else : ?>
          ...
          <div class="skai-card-body">
            <div class="skai-window-shift">
              ...
              <div class="skai-shift-panel">
                ...
                <p class="skai-shift-text"><?php echo htmlspecialchars(aztCommaList(array_slice($top50, 0, 5)), ENT_QUOTES, 'UTF-8'); ?></p>
              </div>

              <div class="skai-shift-panel">
                <p class="skai-shift-label">Broader 300-draw leaders</p>
                <p class="skai-shift-text"><?php echo htmlspecialchars(aztCommaList(array_slice($top300, 0, 5)), ENT_QUOTES, 'UTF-8'); ?></p>
              </div>
            </div>
          </div>
        </div>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The first panel below shows the complete main-number distribution across all values 01&ndash;41. The second panel shows the complete Sixth Ball distribution for the selected bonus window.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        ...
        <div class="skai-grid">
          <div class="skai-card">
            <div class="skai-card-head skai-card-head--radiant">
              Full Sixth Ball distribution
              <span class="skai-card-sub">All values 01&ndash;41 across the last <?php echo (int) $nodCurrentSixth; ?> drawings</span>
            </div>
            <div class="skai-card-body">
              <div class="skai-chart-shell">
                <div class="skai-chart-frame">
                  <canvas id="sixthChart" aria-label="Sixth Ball distribution chart" role="img"></canvas>
                </div>
              </div>
            </div>
          </div>
      ...
      <form name="fqsearch" method="post" action="/all-us-lotteries/results-analysis?st=<?php echo htmlspecialchars((string) $stateAbrev, ENT_QUOTES, 'UTF-8'); ?>&amp;stn=<?php echo htmlspecialchars((string) $stateName, ENT_QUOTES, 'UTF-8'); ?>&amp;gm=<?php echo htmlspecialchars((string) $gName, ENT_QUOTES, 'UTF-8'); ?>#tables">
        <div class="skai-controls-row">
          <div class="skai-controls-left">
            ...
            <label for="pnod">Sixth Ball draw window</label>
            <select name="pnod" id="pnod" class="skai-select">
              <?php foreach (range(10, 700, 5) as $opt) : ?>
                <option value="<?php echo (int) $opt; ?>"<?php echo ((int) $opt === (int) $nodCurrentSixth) ? ' selected="selected"' : ''; ?>>
                  <?php echo (int) $opt; ?>
                </option>
              <?php endforeach; ?>
            </select>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
            ...
            <table id="skai-main-table" class="skai-table" aria-label="Main numbers frequency table">
              ...
              <tbody>
                <?php for ($i = 1; $i <= 41; $i++) : ?>
                  ...
                  list($lastDrawSort, $lastDrawLabel) = aztDrawingsAgoLabel($mainLastSeenIndex[$number] ?? null, (int) $nodCurrentMain);
        ...
        <div class="skai-card">
          ...
          <div class="skai-table-wrap">
            <table id="skai-sixth-table" class="skai-table" aria-label="Sixth Ball frequency table">
              <thead>
                <tr>
                  <th>Number</th>
                  <th>Drawn Times</th>
              ...
              <tbody>
                <?php for ($i = 1; $i <= 41; $i++) : ?>
                  <?php
                  $number = ($i < 10) ? '0' . $i : (string) $i;
                  $countNumber = (int) ($sixthCounts[$number] ?? 0);
                  list($lastDrawSort, $lastDrawLabel) = aztDrawingsAgoLabel($sixthLastSeenIndex[$number] ?? null, (int) $nodCurrentSixth);
                  ?>
                  <tr>
                    ...
                    <td>
                      <input
                        ...
                        type="checkbox"
                        value="<?php echo htmlspecialchars($number, ENT_QUOTES, 'UTF-8'); ?>"
                        aria-label="Track Sixth Ball number <?php echo htmlspecialchars($number, ENT_QUOTES, 'UTF-8'); ?>"
                      >
                    </td>
                  </tr>
                <?php endfor; ?>
          ...
          <div class="skai-tracked">
            <div class="skai-tracked-head">
              <h3 class="skai-tracked-title">Tracked Sixth Ball values</h3>
              <div class="skai-tracked-actions">
      ...
      <div class="skai-tool-grid">
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=AZT">Open SKAI Analysis</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=AZT">Open AI Predictions</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=AZT">Open Skip &amp; Hit</a>
          </div>
        </article>
      </div>

      <div class="skai-utility-grid">
        <a class="skai-utility-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=AZT">MCMC Markov Analysis</a>
        <a class="skai-utility-link" href="/all-lottery-heatmaps?gameId=AZT">Heatmap Analysis</a>
...
(function () {
  ...
  var chartData = {
    ...
    sixthLabels: <?php echo json_encode(array_values($sixthChartLabels)); ?>,
    sixthValues: <?php echo json_encode(array_values($sixthChartValues)); ?>
  };
  ...
  function renderCharts() {
    ...
    var sixthCanvas = document.getElementById('sixthChart');
    ...
    if (sixthCanvas && chartData.sixthLabels.length) {
      new Chart(sixthCanvas.getContext('2d'), {
        type: 'bar',
        data: {
          labels: chartData.sixthLabels,
          datasets: [{
            data: chartData.sixthValues,
            borderWidth: 0,
            borderRadius: 8,
